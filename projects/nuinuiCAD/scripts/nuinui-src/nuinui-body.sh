...
if [ "${NUINUI_LIFECYCLE_RAW:-0}" != 1 ]; then
  ...
  lifecycle_valid_peer() { [ "$1" = FREE ] || nuinui_ownership_valid_issue "$1"; }
  ...
  lifecycle_begin_postmutation_diagnostic() {
    ...
    if [ -n "$lifecycle_begin_diag_repo" ]; then
      ...
      if [ -n "$lifecycle_begin_diag_slot" ] && [ -f "$lifecycle_begin_diag_slot" ]; then
        lifecycle_begin_diag_issue=$(nuinui_ownership_field "$lifecycle_begin_diag_slot" issue 2>/dev/null || true)
        lifecycle_begin_diag_branch=$(nuinui_ownership_field "$lifecycle_begin_diag_slot" branch 2>/dev/null || true)
        lifecycle_begin_diag_base=$(nuinui_ownership_field "$lifecycle_begin_diag_slot" base 2>/dev/null || true)
        lifecycle_begin_diag_claim=$(nuinui_ownership_field "$lifecycle_begin_diag_slot" claim 2>/dev/null || true)
  ...
  lifecycle_start_envelope() {
    ...
    set -- $(nuinui_ownership_field "$lifecycle_start_slot" issue) \
      $(nuinui_ownership_field "$lifecycle_start_slot" branch) \
      $(nuinui_ownership_field "$lifecycle_start_slot" base) \
      $(nuinui_ownership_field "$lifecycle_start_slot" claim) || return 1
    [ "$#" = 4 ] || return 1
    ...
    nuinui_ownership_valid_issue "$lifecycle_start_issue" || return 1
    nuinui_ownership_valid_sha "$lifecycle_start_base" || return 1
    nuinui_ownership_valid_sha "$lifecycle_start_checkpoint" || return 1
  ...
  lifecycle_resume_envelope() {
    ...
    set -- $(nuinui_ownership_field "$lifecycle_resume_slot" issue) \
      $(nuinui_ownership_field "$lifecycle_resume_slot" branch) \
      $(nuinui_ownership_field "$lifecycle_resume_slot" base) \
      $(nuinui_ownership_field "$lifecycle_resume_slot" claim) || return 1
    [ "$#" = 4 ] || return 1
  ...
  lifecycle_release_envelope() {
    ...
    nuinui_ownership_valid_sha "$lifecycle_release_head" || return 1
    nuinui_ownership_valid_sha "$lifecycle_release_origin" || return 1
  ...
  lifecycle_begin() {
    ...
    nuinui_ownership_valid_issue "$lifecycle_begin_issue" || { echo 'ERROR: Issue must look like SAY-123'; return 2; }
    nuinui_ownership_valid_sha "$lifecycle_begin_base" || { echo 'ERROR: expected base must be a full 40-character commit SHA'; return 2; }
    ...
    set -- $(nuinui_ownership_field "$lifecycle_begin_slot" issue) \
      $(nuinui_ownership_field "$lifecycle_begin_slot" branch) \
      $(nuinui_ownership_field "$lifecycle_begin_slot" base) \
      $(nuinui_ownership_field "$lifecycle_begin_slot" claim) || {
        lifecycle_begin_postmutation_diagnostic "$NUINUI_LIFECYCLE_PREFLIGHT_OUTPUT"
  ...
  lifecycle_release_command() {
    ...
    lifecycle_release_validate() {
      ...
      if [ -e "$lifecycle_release_lock_dir" ]; then
        if [ -f "$lifecycle_release_lock_dir/state" ] && nuinui_ownership_parse_lock "$lifecycle_release_lock_dir/state" >/dev/null; then
      ...
      if ! nuinui_ownership_valid_sha "$lifecycle_release_topic_checkpoint" || [ "$lifecycle_release_topic_checkpoint" != "$lifecycle_release_saved_checkpoint" ]; then
        echo 'BLOCKED: checkpoint mismatch'
