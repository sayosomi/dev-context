...
legacy_handoff_owned_by_other_lane() {
  local issue="$1" legacy="$2" candidate session raw_handoff owner_result
  local valid_owner_count=0
  local -a lanes

  lanes=("${(@f)$(lane_manifest_lanes_by_role "$E2E_MANIFEST" human-test)}")
  for candidate in "${lanes[@]}"; do
    [[ -n "$candidate" && "$candidate" != "$E2E_LANE" ]] || continue
    (
      select_human_lane "$candidate" >/dev/null 2>&1 || exit 1
      session="$(session_path)" || exit 1
      path_exists "$session" || exit 1
      raw_handoff="$(metadata_value "$session" handoff 2>/dev/null || true)"
      [[ "$raw_handoff" == "$legacy" ]] || exit 1
      [[ -f "$session" && ! -L "$session" ]] || exit 2
      load_session "$session" || exit 2
      [[ ( "$SESSION_KIND" == current || "$SESSION_KIND" == pre-locale ) &&
        "$SESSION_LANE" == "$candidate" && "$SESSION_ISSUE" == "$issue" &&
        "$SESSION_HANDOFF" == "$legacy" ]] || exit 2
      assert_session_root "$SESSION_ROOT" || exit 2
      assert_persisted_handoff "$SESSION_ISSUE" "$SESSION_HANDOFF" "$SESSION_LANE" || exit 2
      assert_checkout "$issue" "$SESSION_REF" >/dev/null || exit 2
      assert_handoff_file "$SESSION_HANDOFF" "$issue" "$SESSION_REF" \
        "$SESSION_ROOT" "$SESSION_CDP_PORT" "$SESSION_SOURCE_FIXTURE" || exit 2
      assert_process_ownership "$SESSION_ROOT" "$SESSION_LAUNCH_PID" 1 || exit 2
      assert_cdp_reachable "$SESSION_CDP_PORT" || exit 2
    ) >/dev/null 2>&1
    owner_result=$?
    if (( owner_result == 0 )); then
      (( valid_owner_count++ ))
    elif (( owner_result == 2 )); then
      return 1
    fi
  done
  (( valid_owner_count == 1 ))
...
closure_check_lane() {
  ...
  for temp_parent in "${temp_parents[@]}"; do
    ...
    if path_exists "$candidate"; then
      if legacy_handoff_owned_by_other_lane "$requested_issue" "$candidate"; then
        echo "  handoff=owned-by-other-lane:$candidate"
      else
...
cleanup_duplicate() {
  ...
  if [[ "$legacy_handoff" != "$handoff" ]] && path_exists "$legacy_handoff"; then
    legacy_handoff_owned_by_other_lane "$issue" "$legacy_handoff" || {
