...
| scheduled autonomous `only_chatgpt` runner | [Scheduled runner operation](./SCHEDULED-RUNNER.md) |
...
10. **Shared CI incident suspicion / human-terminal Mac reproduction:** `ONLY-CHATGPT.md`のshared CI incident routeに該当した場合だけ`CI-INCIDENTS.md`を読む。通常の`only_chatgpt`開始時やordinary issue-local CI failureでは読まない。
